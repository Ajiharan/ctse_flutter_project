...
import 'package:ctse_app/model/assignment.dart';
...
class _ViewAssignmentsState extends State<ViewAssignments> {
  ...
  void saveAssignment() async{
    Map <String, dynamic> assignmentData = {"id":id.text, "title":title.text, "description":description.text};
    await FirebaseFirestore.instance.collection("assignments").add(assignmentData).catchError((e){
      print(e.toString());
    });
    uploadFile();
  }
                                ...
                                GestureDetector(
                                  onTap: (){
                                    ...
                                    Scaffold.of(context).showSnackBar(
                                      SnackBar(
                                          content: Text("Assignment details saved!", style: TextStyle(fontSize: 15, color:Colors.indigo),)
                                      ),
                                    );
                                  },
                                  child: Container(
  ...
  Widget _buildBody(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
        stream:FirebaseFirestore.instance.collection("assignments").orderBy("id", descending:true).snapshots(),
  ...
  DataRow _buildRowItem(BuildContext context, DocumentSnapshot data){
    final assignment  = Assignment.fromSnapshot(data);
          ...
          builder: (context){
            return AlertDialog(
              ...
              actions: [
                ...
                FlatButton(
                    onPressed: (){
                      FirebaseFirestore.instance.collection("asignments").doc(id).delete().whenComplete(() => Navigator.pop(context));
                    },
    ...
    return DataRow(cells: [
      DataCell(Text(assignment.id)),
      DataCell(Text(assignment.title)),
      DataCell(Text(assignment.description)),
      DataCell(
        Row(
          children: [
            IconButton(
            ...
            IconButton(
              onPressed: (){
                showDeleteDialog(assignment.id);
              },
